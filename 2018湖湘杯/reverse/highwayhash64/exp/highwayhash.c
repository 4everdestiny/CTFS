...
void HighwayHashReset(const uint64_t key[4], HighwayHashState* state) {
  ...
  state->v0[0] = 0xCF0C0C1ED5EDF3Eull; //state->mul0[0] ^ key[0];
  state->v0[1] = state->mul0[1] ^ 0x3F3E3D3C3B3A1918ull; //state->mul0[1] ^ key[1];
  state->v0[2] = state->mul0[2] ^ 0x1226252423222121ull; //state->mul0[2] ^ key[2];
  state->v0[3] = state->mul0[3] ^ 0x2F2E2D2C2B2A2928ull; //state->mul0[3] ^ key[3];
  state->v1[0] = state->mul1[0] ^ 0x1312111117161514ull; //state->mul1[0] ^ ((key[0] >> 32) | (key[0] << 32));
  state->v1[1] = state->mul1[1] ^ 0x3B3A19183F3E3D3Cull; //state->mul1[1] ^ ((key[1] >> 32) | (key[1] << 32));
  state->v1[2] = state->mul1[2] ^ 0x2322212112262524ull; //state->mul1[2] ^ ((key[2] >> 32) | (key[2] << 32));
  state->v1[3] = state->mul1[3] ^ 0x2B2A29282F2E2D2Cull; //state->mul1[3] ^ ((key[3] >> 32) | (key[3] << 32));
}
